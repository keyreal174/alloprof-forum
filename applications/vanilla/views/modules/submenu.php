...
?>
<ul class="Question-submenu <?php echo $additionalClass ?>">
...
%text</a>'); ?>
    ...
    <li class='Navigation-linkContainer header__link'>
        <?php echo Gdn_Theme::link('discussions/bookmarked', sprite('Home').' '.'<span class="desktop link__text">'.t('Follow ups').($followedDiscussionsCount > 0 ? "<span class='realcount Count'>".$followedDiscussionsCount."</span>" : '').'</span>'.'<span class="mobile">'.t('Follow ups'), '<a href="%url" class="%class Navigation-link link link--header"><br/>
%text</a>'); ?>
    </li>
    <li class='Navigation-linkContainer mobile'>
...
%text</a>'); ?>
    ...
    <?php
            if(Gdn::session()->isValid()){
                ...
                echo "<li class='Navigation-linkContainer header__link'>";
                echo Gdn_Theme::link('discussions/mine', sprite('Home').' '.'<span class="link__text">'.t('My Questions').'</span>', '<a href="%url" class="%class Navigation-link link link--header"><svg class="d-mobile" width="21" height="22" viewBox="0 0 21 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <?php
            if(Gdn::session()->isValid()){
                ...
                echo "<li class='Navigation-linkContainer header__link'>";
                echo Gdn_Theme::link('discussions/bookmarked', sprite('Home').' '.'<span class="link__text">'.t('Follow ups').'</span>', '<a href="%url" class="%class Navigation-link link link--header"><svg class="d-mobile" width="19" height="21" viewBox="0 0 19 21" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <li class='Navigation-linkContainer d-desktop header__link'>
        <?php
               echo '<a class="Navigation-link link link--header ToggleFlyout Flayout-Subject Subject-toggle" rel="/categories/subjectdropdown">';
               echo '<span class="link__text">'.t("Topics").'</span>';
