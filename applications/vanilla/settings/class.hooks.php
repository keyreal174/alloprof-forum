...
class VanillaHooks extends Gdn_Plugin {
    ...
    public function discussionsController_addProfileTabsInfo_handler($sender) {
        if (is_object($sender->User) && $sender->User->UserID > 0) {
            ...
            $sender->addProfileTab('Questions asked', userUrl($sender->User, '', 'discussions'), 'Questions', $discussionsLabel);
            $sender->addProfileTab('Explanations received', userUrl($sender->User, '', 'comments'), 'Comments', $commentsLabel);
            // Add the discussion tab's CSS and Javascript.
            $sender->addJsFile('jquery.gardenmorepager.js');
            $sender->addJsFile('discussions.js', 'vanilla');
        }
