...
jQuery(document).ready(function ($) {
  ...
  $(document).on('click', 'a.WriteButton', function () {
    if ($(this).hasClass('WriteButton')) {
      ...
      frm.trigger('WriteButtonClick', [frm]); // Reveal the "Preview" button and hide this one
  ...
  $('#CommentForm :submit').click(function () {
    var btn = this;
    var frm = $(btn).parents('form').get(0); // Handler before submitting
    ...
    $.ajax({
      ...
      error: function error(XMLHttpRequest, textStatus, errorThrown) {
        ...
        $('div.Popup').remove(); // Add new popup with error

        $.popup({}, XMLHttpRequest.responseText);
      },
      success: function success(json) {
        // Remove any old popups if not saving as a draft
        if (!draft) $('div.Popup').remove(); // Assign the comment id to the form if it was defined
        ...
        if (json.DraftID != null && json.DraftID != '') $(inpDraftID).val(json.DraftID); // Remove any old errors from the form
        ...
        if (json.FormSaved == false) {
          ...
          $(frm).triggerHandler('complete'); // Redirect to the discussion
  ...
  $(document).on('click', '#DiscussionForm :submit', function () {
    ...
    $.ajax({
      ...
      success: function success(json) {
        ...
        if (json.DiscussionID != null) $(inpDiscussionID).val(json.DiscussionID);
        if (json.DraftID != null) $(inpDraftID).val(json.DraftID); // Remove any old errors from the form
        ...
        if (json.FormSaved == false) {
          ...
          if (json.RedirectTo) {
            $(frm).triggerHandler('complete'); // Redirect to the new discussion
