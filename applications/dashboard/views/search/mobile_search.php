...
<div class="modal-body">
    <h2><?php echo t('Explore by subject') ?></h2>
    <ul class="mobile-categories">
        <?php
            $MaxDepth = c('Vanilla.Categories.MaxDisplayDepth');
            $dbRecords = writeCategories();

            foreach ($dbRecords as $CategorySet) {
                echo '<li class="categoryset '.$CssClass.'" '.$attributes.'>';
                foreach ($CategorySet as $Category) {
                    ...
                    echo '<div class="ClearFix '.$CssClass.'" '.$attributes.'>';
